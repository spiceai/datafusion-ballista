    ...
    async fn acquire(
        ...
        let client = match maybe_idle_client {
            ...
            None => {
                ...
                // Fork delta: connect via `connect_ballista_client` so the
                // channel's transport tasks run on the registered shuffle
                // transport runtime rather than the CPU-saturated task runtime.
                connect_ballista_client(host, port, config, customize_endpoint).await?
            }
        };

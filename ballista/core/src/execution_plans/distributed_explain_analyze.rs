...
/// This operator fetches stage metrics from the Ballista scheduler after the query
/// has been executed in children (DistributedQueryExec).
#[derive(Debug, Clone)]
...
async fn fetch_job_metrics(
    ...
    let mut endpoint = create_grpc_client_endpoint(scheduler_url.to_string())
...
mod tests {
    ...
    fn test_format_metrics_as_record_batch() {
        ...
        let expected = [
            ...
            "ProjectionExec: expr=[a], metrics=[output_rows=4, elapsed_compute=15ms]",
            "",

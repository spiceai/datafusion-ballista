...
/// Ballista wrapper around `LogicalPlan::Explain`.
///
/// `datafusion-proto` serializes `LogicalPlan::Explain` through its built-in
/// `ExplainNode`, which discards the `explain_format` field (the variant
/// selected by `EXPLAIN FORMAT TREE` / `EXPLAIN FORMAT JSON` / etc.). The
/// client wraps the `Explain` in this extension before serialization, and the
/// scheduler unwraps it (see `unwrap_ballista_explain` in
/// `scheduler::state::mod`) into a native `LogicalPlan::Explain` before
/// physical planning, so DataFusion renders the user-requested format.
#[derive(Debug, Clone)]
pub struct BallistaExplainNode {
    /// Whether `EXPLAIN VERBOSE` was requested.
    pub verbose: bool,
    /// The explain output format. Stored as a stable string so changes to the
    /// `ExplainFormat` enum on either side do not break the wire format.
    pub explain_format: String,
    /// The plan being explained.
    pub plan: Arc<LogicalPlan>,
    /// Output schema for `EXPLAIN`: `(plan_type, plan)`.
    pub schema: DFSchemaRef,
}

impl BallistaExplainNode {
    /// Stable string identifier for an `ExplainFormat`. Must round-trip with
    /// [`Self::format_from_str`].
    pub fn format_as_str(
    ...
    /// Inverse of [`Self::format_as_str`]. Returns `None` for unknown
    /// identifiers, so the codec can fall through to the default handler.
    pub fn format_from_str(s: &str) -> Option<datafusion::common::format::ExplainFormat> {
        use datafusion::common::format::ExplainFormat;
        match s {
...
impl std::hash::Hash for BallistaExplainNode {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        self.verbose.hash(state);
        self.explain_format.hash(state);
    ...
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        match self.verbose.partial_cmp(&other.verbose) {
            Some(std::cmp::Ordering::Equal) => {
                match self.explain_format.partial_cmp(&other.explain_format) {
                    Some(std::cmp::Ordering::Equal) => self.plan.partial_cmp(&other.plan),
                    other => other,
                }
            }
            other => other,
...
impl UserDefinedLogicalNodeCore for BallistaExplainNode {
    ...
    fn fmt_for_explain(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(
            f,
            "BallistaExplainNode: verbose={}, format={}",
            self.verbose, self.explain_format
        )
    }
    ...
    fn with_exprs_and_inputs(
        ...
        let [input] = <[LogicalPlan; 1]>::try_from(inputs).map_err(|_| {
            datafusion::error::DataFusionError::Plan(
                "BallistaExplainNode expects exactly 1 input".to_string(),
            )
        })?;
        Ok(Self {
